...
from rebalancing_strategies import (
    ...
    EqualWeightStrategy, SpyOnlyStrategy
)
...
class Portfolio:
    ...
    def __init__(self,
                 ...
                 name: str = "portfolio"):
        """
        Initialize portfolio with strategy and initial weights.
        ...
        rebalancing_strategy : BaseRebalancingStrategy
            Strategy object controlling rebalancing behavior
        optimizer : PortfolioOptimizer, optional
            Optimizer instance (required for optimization-based strategies)
        optimization_method : str, optional
            Optimization method name (e.g., 'mean_variance')
        name : str
    ...
    def create_target_weight(cls,
        ...
        rebalance_days : int
            Days between rebalancing (default: 30)
        name : str
            Portfolio name
        ...
        return cls(asset_names, target_weights, strategy, name=name)
    ...
    def create_optimized(cls,
        ...
        from rebalancing_strategies import OptimizedRebalancingStrategy
        ...
        return cls(asset_names, initial_weights, strategy,
                  optimizer=optimizer, optimization_method=method, name=name)
    ...
    def run_backtest(self, period_manager) -> None:
        """
        Run backtest across all rebalancing periods.
        ...
        for period_num, period_data, period_info in period_manager.iter_periods():
            ...
            # Handle rebalancing at period start
            if period_num == 0:
                # First period - use initial weights
                rebalanced_weights = current_weights.copy()
            elif isinstance(self.rebalancing_strategy, BuyAndHoldStrategy):
                # Buy-and-hold: calculate drifted weights from previous period
                if period_num > 0:
                ...
                # Other strategies: call rebalancing strategy
                lookback_data = None
                if hasattr(self.rebalancing_strategy, 'optimizer'):
                    # Optimization strategy needs historical data
                    lookback_data = period_manager.get_expanding_window_data(period_num)

                # Get target weights from strategy
                target_weights_array = self.rebalancing_strategy.calculate_target_weights(
                    current_weights=current_weights.values,
                    period_returns=period_data.mean(),  # Use mean as proxy
                    current_date=period_end.date(),
                    lookback_returns=lookback_data,
                    target_weights=self.current_weights.values
                )

                rebalanced_weights = pd.Series(target_weights_array, index=self.asset_names)
