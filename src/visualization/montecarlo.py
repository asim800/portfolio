...
def run_accumulation_mc(
    ...
    assert freq_ratio.is_integer(), f"asset_returns_frequency {asset_returns_frequency} / withdrawals_per_year {contributions_per_year} is not an integer"
    freq_ratio = int(freq_ratio)

    if freq_ratio > 1:
      # Expand asset returns to match withdrawal frequency
      # Reshape to split each period into freq_ratio sub-periods
      # Original shape: (num_simulations, total_periods, num_assets)
      # New shape: (num_simulations, total_periods * freq_ratio, num_assets)
      asset_returns_paths = np.expand_dims(asset_returns_paths, axis=2)  # Add dimension for splitting
      asset_returns_paths = asset_returns_paths.reshape(num_simulations, expected_periods, int(freq_ratio), weights.shape[0])  # Split periods
      asset_returns_paths = asset_returns_paths.sum(axis=2).squeeze()  # sum sub-period returns
    ...
    for period in range(1, expected_periods + 1):
        ...
        # Calculate portfolio returns (vectorized matrix multiplication)
        # portfolio_returns[i] = dot(weights, asset_returns[i, :])
        ...
        # Add contributions (vectorized across all simulations)
        if contribution_amount > 0:
            portfolio_values += contribution_amount

            # Calculate employer match (same for all simulations)
            period_match = contribution_amount * employer_match_rate

            # Apply annual cap (deterministic, same for all simulations)
...
def run_decumulation_mc(
    ...
    asset_returns_paths : np.ndarray
        ...
        Shape: (num_simulations, total_periods, num_assets)
        where total_periods = years * withdrawals_per_year
    annual_withdrawal : float
    ...
    assert freq_ratio.is_integer(), f"asset_returns_frequency {asset_returns_frequency} / withdrawals_per_year {withdrawals_per_year} is not an integer"
    freq_ratio = int(freq_ratio)
    ...
    total_periods = asset_returns_paths.shape[1]  # Update total periods after reshaping
    # Validate
    ...
    if total_periods != expected_periods:
        raise ValueError(f"asset_returns_paths has {total_periods} periods, expected {expected_periods} "
                        f"(years={years} × withdrawals_per_year={withdrawals_per_year})")

    # Calculate per-period withdrawal amount
    ...
    # Run simulations period-by-period
    for sim in range(num_simulations):
        ...
        for period in range(1, expected_periods + 1):
            ...
            # Calculate inflation-adjusted withdrawal (based on current year)
